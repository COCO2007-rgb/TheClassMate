...
# Expose specific collection helpers
...
def run_db_migration():
    ...
    try:
        # 1. Ensure default coaching center
        ...
        if not dev_user:
            ...
            print("Default developer account created successfully.")
        
        # 2. Migrate existing users (admins/teachers)
        users_col.update_many(
            {"role": {"$ne": "parent"}, "coaching_center_id": {"$exists": False}},
            {"$set": {"coaching_center_id": center_str_id}}
        )
        
        # 3. Migrate existing batches
        ...
        # 5. Migrate existing payments
        payments_col.update_many(
            {"coaching_center_id": {"$exists": False}},
            {"$set": {"coaching_center_id": center_str_id}}
        )
        
        # 6. Set defaults on existing payments
        ...
        print("Database schema normalized and migration executed successfully.")
